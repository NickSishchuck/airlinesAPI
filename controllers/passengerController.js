...
exports.getPassenger = asyncHandler(async (req, res, next) => {
  ...
  if (!passenger) {
    return next(
      new ErrorResponse(`Passenger not found with id of ${req.params.id}`, 404),
    );
  }
  ...
  if (
    req.user.role !== "admin" &&
    req.user.role !== "staff" &&
    req.user.passenger_id != req.params.id
  ) {
    return next(
...
exports.updatePassenger = asyncHandler(async (req, res, next) => {
  ...
  if (
    ...
    const exists = await Passenger.passportExists(
      req.body.passport_number,
      req.params.id,
    );
...
exports.deletePassenger = asyncHandler(async (req, res, next) => {
  ...
  if (tickets.length > 0) {
    return next(
      new ErrorResponse("Cannot delete passenger with booked tickets", 400),
    );
  }
...
exports.getPassengerTickets = asyncHandler(async (req, res, next) => {
  ...
  if (req.user.role !== "admin" && req.user.passenger_id != req.params.id) {
    return next(
      new ErrorResponse("Not authorized to access these tickets", 403),
    );
  }
